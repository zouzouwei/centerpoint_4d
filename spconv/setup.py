...
PYBIND11_CMAKE_DIR = str(Path(pybind11.__file__).resolve().parent / "share" / "cmake" / "pybind11")
...
BUILD_JOBS = os.environ.get("SPCONV_BUILD_JOBS", "4")
...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        cmake_args = [# '-G "Visual Studio 15 2017 Win64"',
                      '-DCMAKE_PREFIX_PATH={}'.format(';'.join([LIBTORCH_ROOT, PYBIND11_CMAKE_DIR])),
        ...
        if CUDA_ARCHITECTURES:
            cmake_args.append('-DCMAKE_CUDA_ARCHITECTURES={}'.format(CUDA_ARCHITECTURES))
