...
namespace spconv {
template <typename T>
__device__ __forceinline__ bool greater_than(const T &lhs, const T &rhs) {
  return lhs > rhs;
}

template <>
__device__ __forceinline__ bool greater_than<at::Half>(const at::Half &lhs,
                                                       const at::Half &rhs) {
  return __hgt(lhs.operator __half(), rhs.operator __half());
}

template <typename T>
__device__ __forceinline__ bool equal_to(const T &lhs, const T &rhs) {
  return lhs == rhs;
}

template <>
__device__ __forceinline__ bool equal_to<at::Half>(const at::Half &lhs,
                                                   const at::Half &rhs) {
  return __heq(lhs.operator __half(), rhs.operator __half());
}
